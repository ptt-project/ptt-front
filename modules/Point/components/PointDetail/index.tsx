      ...
      return (
        <div className="mb-3">
          <Row className={`mt-4 ${styles.tableTitle}`}>
            <Col span={18}>{t('shopPoint.user')} : <Avatar src="https://joeschmoe.io/api/v1/random" /> {item.title}</Col>
            <Col className="text-left mt-1" span={6}>{t('shopPoint.orderID')}: <Text>{item.orderID}</Text></Col>
          </Row>
          <Row >
            ...
            <Col className={styles.replyCol} span={6}>
              {item.status === 'reply'?
              <Button className="mt-1">{t('shopPoint.reply')}</Button> 
            :<Text>{item.productName}</Text>}
            </Col>
          </Row>
        </div>
